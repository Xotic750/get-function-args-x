 ...
 */
"use strict";var isFunctionLike,forEach=_dereq_("array-for-each-x"),trim=_dereq_("trim-x"),fToString=Function.prototype.toString,ARROW_ARG=/^([^(]+?)=>/,s=_dereq_("white-space-x").ws,FN_ARGS=new RegExp("^[^\\(]*\\(["+s+"]*([^\\)]*)\\)","m"),STRIP_COMMENTS=/((\/\/.*$)|(\/\*[\s\S]*?\*\/))/gm;try{new Function('"use strict"; return class My {};')(),isFunctionLike=function _isFunctionLike(value){return"function"==typeof value}}catch(ignore){isFunctionLike=_dereq_("is-function-x")}module.exports=function getFunctionArgs(fn){if(!1!==isFunctionLike(fn)){var str;try{str=fToString.call(fn).replace(STRIP_COMMENTS," ")}catch(ignore){return""}var match=str.match(ARROW_ARG)||str.match(FN_ARGS),arr=[];return match&&forEach(match[1].split(","),function reducer(item){var a=trim(item);a&&arr.push(a)}),arr}}},{"array-for-each-x":2,"is-function-x":11,"trim-x":34,"white-space-x":36}],2:[function(_dereq_,module,exports){/**
