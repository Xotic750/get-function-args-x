 ...
 */
"use strict";var reduce=_dereq_("array-reduce-x"),replaceComments=_dereq_("replace-comments-x"),normalise=_dereq_("normalize-space-x"),trim=_dereq_("trim-x"),fToString=Function.prototype.toString,ARROW_ARG=/^([^(]+?)=>/,FN_ARGS=/^[^(]*\( *([^)]*)\)/m,isFunction=_dereq_("is-function-x");module.exports=function getFunctionArgs(fn){if(!1!==isFunction(fn,!0)){var str;try{str=normalise(replaceComments(fToString.call(fn)," "))}catch(ignore){return""}var match=str.match(ARROW_ARG)||str.match(FN_ARGS);return reduce(match?match[1].split(","):[],function reducer(acc,item){var a=trim(item);return a&&acc.push(a),acc},[])}}},{"array-reduce-x":2,"is-function-x":9,"normalize-space-x":21,"replace-comments-x":24,"trim-x":34}],2:[function(_dereq_,module,exports){/**
 ...
 */
"use strict";var isSymbol=_dereq_("is-symbol"),pToString=_dereq_("has-symbol-support-x")&&Symbol.prototype.toString;module.exports=function safeToString(value){return pToString&&isSymbol(value)?pToString.call(value):String(value)}},{"has-symbol-support-x":6,"is-symbol":17}],27:[function(_dereq_,module,exports){/**
 ...
 * @module to-integer-x
 */
"use strict";var $isNaN=_dereq_("is-nan"),$isFinite=_dereq_("is-finite-x"),$sign=_dereq_("math-sign-x");module.exports=function ToInteger(value){var number=Number(value);return $isNaN(number)?0:0===number||!1===$isFinite(number)?number:$sign(number)*Math.floor(Math.abs(number))}},{"is-finite-x":8,"is-nan":11,"math-sign-x":19}],28:[function(_dereq_,module,exports){/**
 ...
 * @module to-length-x
 */
"use strict";var toInteger=_dereq_("to-integer-x"),MAX_SAFE_INTEGER=_dereq_("max-safe-integer");module.exports=function ToLength(value){var len=toInteger(value);return len<=0?0:len>MAX_SAFE_INTEGER?MAX_SAFE_INTEGER:len}},{"max-safe-integer":20,"to-integer-x":27}],29:[function(_dereq_,module,exports){/**
